    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            for d in range(D):
                ...
                # error = len([y for y in leftY if y != util.mode(leftY)]) + len([y for y in rightY if y != util.mode(rightY)])   ### TODO: YOUR CODE HERE
                # error = size((leftY != util.mode(leftY)).nonzero()) + size((rightY != util.mode(rightY)).nonzero())
                # error = sum(leftY != util.mode(leftY)) + sum(rightY != util.mode(rightY))
                leftError = sum(leftY != util.mode(leftY)) if len(leftY) > 0 else 0    ### TODO: YOUR CODE HERE
                rightError = sum(rightY != util.mode(rightY)) if len(rightY) > 0 else 0
