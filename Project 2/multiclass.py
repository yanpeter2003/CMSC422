...
class MCTree:
    ...
    def train(self, X, Y):
        for n in self.tree.iterNodes():
            ...
            ### TODO: YOUR CODE HERE
            thisX = X[(Y == leftLabels[0]) | (Y == rightLabels[0])]
            thisY = (Y[(Y == leftLabels[0]) | (Y == rightLabels[0])] == rightLabels[0]) * 2 - 1
    ...
    def predict(self, X):
        ...
        while not current_node.isLeaf:
            classifier = current_node.getNodeInfo()
            prediction = classifier.predict_proba(X.reshape(1, -1))

            # Navigate the tree based on the prediction
            if prediction[0, 1] > 0.5:  # If the probability of the right class is greater than 0.5, go right
